...
var server = (config) => {
  ...
  if (!config.listeningIps) {
    ...
    Object.keys(ifaces).forEach((ifaceName) => {
      ...
      iface.forEach((network) => {
        this.listeningIps.push(network.address);
      });
    });
  }
  ...
  this.on('message', (msg) => {
    this.debug('DEBUG','Receiving ' + msg);
    ...
    this.authentification.auth(msg, (err, reply) => {
      if (err) {
        this.debug('DEBUG', 'authentification failled for TransactionID: ' + msg.transactionID);
        this.debug('TRACE', err);
        return;
      }
      this.emit(msg.getMethodName(), msg, reply);
    });

  });

  this.on('binding', (msg, reply) => {
...
server.prototype.debug = (level, msg) => {
...
server.prototype.addUser = (username, password) => {
  this.authentification.addUser(username, password);
};

server.prototype.removeUser = (username) => {
