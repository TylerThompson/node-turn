  ...
  if (name) {
    ...
    switch (this.type) {
      ...
      case CONSTANTS.ATTR.XOR_RELAYED_ADDRESS:
        this.length = (this.value.family === CONSTANTS.TRANSPORT.FAMILY.IPV6) ? 20 : 8;
        break;
...
attribute.prototype.read = function(data) {
  ...
  switch (this.type) {
    ...
    case CONSTANTS.ATTR.MAPPED_ADDRESS:
      ...
      port = data.readUInt16BE(6) ^ (this.msg.magicCookie >> 16);
      address = [];
      switch (family) {
        case CONSTANTS.TRANSPORT.FAMILY.IPV4:
          ...
          address.push(data[8] ^ magicCookieBuffer[0]);
          address.push(data[9] ^ magicCookieBuffer[1]);
          address.push(data[10] ^ magicCookieBuffer[2]);
          address.push(data[11] ^ magicCookieBuffer[3]);
          this.value = new Address(address.join('.'), port);
          break;
        case CONSTANTS.TRANSPORT.FAMILY.IPV6:
          var magicCookieTransIDBuffer = Buffer.alloc(16);
          magicCookieTransIDBuffer.writeUInt32BE(this.msg.magicCookie);
          magicCookieTransIDBuffer.write(this.msg.transactionID, 4, 12, 'hex');
          for (i = 0; i < 8; i++) {
            var xored = data.readUInt16BE(i * 2 + 8) ^ magicCookieTransIDBuffer.readUInt16BE(i * 2);
            address.push(xored.toString(16));
          }
          this.value = new Address(address.join(':'), port);
          break;
        default:
    ...
    case CONSTANTS.ATTR.MAPPED_ADDRESS:
      ...
      */
      ...
      if (this.value.family === CONSTANTS.TRANSPORT.FAMILY.IPV4) {
        ...
        attrValue[4] = ip[0] & 0xFF;
        attrValue[5] = ip[1] & 0xFF;
        attrValue[6] = ip[2] & 0xFF;
        attrValue[7] = ip[3] & 0xFF;
      } else {
        ip = this.value.address.split(':');
        for (var i in ip) {
          attrValue.writeUInt16BE(parseInt(ip[3], 16), i * 2 + 4);
        }
      }
      break;
    ...
    case CONSTANTS.ATTR.XOR_RELAYED_ADDRESS:
      ...
      */
      ...
      if (this.value.family === CONSTANTS.TRANSPORT.FAMILY.IPV4) {
        ...
        ipaddr.forEach((byte, i) => {
          attrValue[i + 4] = (byte & 0xff) ^ magicCookieBuffer[i];
        });
      } else {
        var magicCookieTransIDBuffer = Buffer.alloc(16);
        magicCookieTransIDBuffer.writeUInt32BE(this.msg.magicCookie);
        magicCookieTransIDBuffer.write(this.msg.transactionID, 4, 12, 'hex');
        ipaddr = this.value.address.split(':');
        ipaddr.forEach((word, i) => {
          attrValue.writeUInt16BE(parseInt(word, 16) ^ magicCookieTransIDBuffer.readUInt16BE(i * 2), i * 2 + 4);
        });      
      }
      break;
